...
$pagination->records($user->countUsers());
...
<div class="panel panel-default">
	...
	<form action="" method="POST">
		...
		<table class="table table-bordered table-striped dataTable" id="datatable1" width="100%">
			...
			<tbody>
				<?php
					$offset = ($pagination->get_page()-1)*$per_page;
					if($listUsers = $user->GetUsers($offset,$per_page)){
						foreach($listUsers as $u){
							$status =  $u->active == 0 ? lang('INACTIVE') : lang('ACTIVE');
							$JoinedOn = new datetime($u->signup);
							if($role = $roles->getRoleById($u->roles)){
								$role = $role->getName();
							}else{
								$role = lang("Undefined");
							}
		...
		<div class="modal-content">
			...
			<div class="modal-body">
					...
					<select name="roles" id="roles" class="form-control">
						<option value="0">-- <?php echo lang('Choose the user role'); ?> --</option>
						<?php
						foreach ($roles->getRoles() as $role) { ?>

							<option value="<?php echo $role->getId(); ?>"><?php l($role->getName()); ?></option>
						<?php } ?>
					</select>
			</div>

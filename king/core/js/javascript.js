...
*/
function savePost(){
	if($.trim($("#postForm #message").val()) == "" && $.trim($("#postForm #link").val()) == "" && $.trim($("#postForm #imageURL").val()) == "" && $.trim($("#postForm #video").val()) == ""){
		alertBox(lang('POST_EMPTY'),"danger","#postForm .messageBox",true,true);
	}else{
		if($("#postForm #postId").val() != "") {
			...
			$.post(
				"ajax/savepost.php",
				{
					...
					descriptionVideo:$("#postForm #video-description").val(),
					action: 'update'
				},
				function(data){
					if($.trim(data) == "true"){
...
$( document ).ready(function() {
	...
	$("#postForm #post").click(function(){
		...
		if($("#postForm #postType").val() == "message" && $.trim($("#postForm #message").val()) == ""){
			...
			alertBox(lang('POST_EMPTY'),"danger","#postForm .messageBox",true);	
		}else if($("#postForm #postType").val() == "image" && $.trim($("#postForm #imageURL").val()) == ""){
			alertBox(lang('POST_EMPTY'),"danger","#postForm .messageBox",true);	
		}else{
			post();
		}
...
function send()
{
	...
	params["postType"] = $("#postForm #postType").val();
		...
		if($("#postForm #postType").val() == "image"){
			if($.trim($("#postForm #imageURL").val()) != ""){
				params["image"] = $("#postForm #imageURL").val();
			}
		}
		...
		if($("#postForm #postType").val() == "video"){
			...
			if($.trim($("#postForm #video-description").val()) != ""){
				params["descriptionVideo"] = $("#postForm #video-description").val();
			}
		}
		...
		$.ajax({
	        ...
	        success: function( data, textStatus, jQxhr ){
			  	if(data.status == "success"){
					...
					$(".postStatus_"+currentGroup).html("<a href='https://www.facebook.com/"+data.id+"' target='_blank'><span class='glyphicon glyphicon-ok'></span> "+lang('VIEW_POST')+" </a>");
				}else{
					$('#'+currentGroup).removeClass('postingSuccess');
					$('#'+currentGroup).addClass('postingError');
					$(".postStatus_"+currentGroup).html("<span class='glyphicon glyphicon-info-sign'></span> "+data.message);
				}
	        },
	        error: function( jqXhr, textStatus, errorThrown ){
	        	$('#'+currentGroup).removeClass('postingSuccess');
				$('#'+currentGroup).addClass('postingError');
				$(".postStatus_"+currentGroup).html("<span class='glyphicon glyphicon-info-sign'></span> "+textStatus);
	        }
	    });
}
/*
...
function post(){
	...
	if (POST_IN_PROGRESS == false) {
		...
		alertBox(lang('POST_IN_PROGRESS_REFRESH_PAGE_TO_CANCEL'),"danger",false,true,true);
	}
}
	...
	$("#postForm #saveScheduledPost").click(function(){
			...
			if($.trim($("#postForm #message").val()) == "" && $.trim($("#postForm #link").val()) == "" && $.trim($("#postForm #imageURL").val()) == "" && $.trim($("#postForm #video").val()) == ""){
				...
				"ajax/savescheduledposts.php",
				{
						...
						resume_after: $("#scheduleResumeAfter","#postForm").val(),
				},
				function(data){
					if(data == "true"){

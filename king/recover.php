...
// Validate the recover code & email
		if(Input::Get("email","GET") && Input::Get("code","GET")){ 
			if($recover = $user->UserResetPWCode(Input::Get("email","GET"),Input::Get("code","GET"))){
				?>
				...
				echo "<div class='alert alert-danger' role='alert'>";
				echo "<span class='glyphicon glyphicon-exclamation-sign' aria-hidden='true'></span>";
				echo lang("Invalid recover code.");
				echo "</div>";
			}
					...
					if(Token::check(Input::get('token'))){
						...
						$validation = $validate->check($_POST, array(
							'email' => array(
								'disp_text' => 'E-mail',
								'required' => true,
								'exists' => array('table' => 'users', 'column' => 'email')
								)
							));
						...
						if($validation->passed()){
							...
							try{
								DB::GetInstance()->Query("UPDATE users set reset_pw_code = ? WHERE email = ? ",array($code,Input::Get("email")));

								$recoverMessage = lang("You recently requested a new password")."<br /><br />";
								$recoverMessage .= lang("Please click the link below to complete your request")."<br/>";
								...
								Mail::Send(Input::Get("email"),lang('Account recovry request'),$recoverMessage);

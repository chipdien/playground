...
if(Input::Get("postType",'POST') && Input::Get("post_title",'POST') && Input::Get("action",'POST') == 'add'){
	...
	if(Input::Get("postType") == "link" ){
		...
		$params['descriptionVideo']= trim(Input::Get("descriptionVideo"));
	}
	
	try{
		echo Posts::SavePost($params,Input::Get("postType"),Input::Get("post_title"));
	}catch(Exception $ex){
		echo $ex->GetMessage();
	}
...
if(Input::Get("postType",'POST') && Input::Get("postId",'POST') && Input::Get("action",'POST') == 'update'){
	...
	try{
		if(Posts::UpdatePost($params,Input::Get("postType"),Input::Get("postId"))){
			echo "true";
		}else{
